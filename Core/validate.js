...
module.exports = function (Config, Web3, Rabbot) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(trxConfirmations > 0 && typeof message.firstValidation === "undefined"){
                    ...
                    const actionSender = new ActionSender(messageFirstValidation.project);
    ...
    let transactionPendingReceiver;
    ...
    const transactionSender = new TransactionsSender();
    transactionPendingReceiver = new TransactionPendingReceiver();

    transactionPendingReceiver.receive(async function (message, properties, actions_, next) {
                ...
                .catch(async (error) => {
                    if (typeof error.code !== "undefined" && error.code === ErrorWeb3.TRANSACTION_ERROR) {
                        ...
                        if (new Date() - properties.timestamp < 5000) {
                            ...
                            // Always reject forever
                            console.error(error);
                            actions.reject();
                        }
                    } else if (typeof error.code !== "undefined" && error.code === ErrorWeb3.TRANSACTION_PENDING_IN_QUEUE) {
                        // Resend to the queue
                        console.log('  Send validate back to queue (still pending): ' + message.transactionId + ". Transaction hash: " + message.hash);
                        actions.nack();
                    } else {
                        // Resend to the queue
